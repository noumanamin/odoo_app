...
import 'package:odoo_app/odoo_helpers/odoo_version.dart';
...
class Odoo {
  ...
  Future<http.Response> authenticate(
    ...
    final res = await callKW("ir.ui.menu", "search_read", []);
    menuResponse = res;
  ...
  Future<OdooResponse> searchRead(
    ...
    var url1 = createPath("/web/dataset/crm.lead");
    var params = {
      "context": getContext(),
      "domain": domain,
      "fields": fields,
    ...
    // OdooResponse data = await callKW("crm.lead", "search_read", fields);
    // var ob = data.getResult();

    // var res = await write(model,[743], {'name':'AAABDUL QAYYUM kkk'});
    // var res = await callKW("product.product", 'search_read', []);
    // res.getResult();

    // return res;
    return await callRequest(model, createPayload(params));
  }
  ...
  getCrmLead(String url, Map<dynamic, dynamic> payload) async {
